...
(function(exports) {
    ...
    Trans2D.prototype.clear = function(x,y) {
        var that = this;
        that.mat = [[1,0,0],[0,1,0],[0,0,1]];
        that.matInv = [[1,0,0],[0,1,0],[0,0,1]];
    }
    Trans2D.prototype.translate = function(x,y) {
        var that = this;
        var mat2 = [[1,0,x],[0,1,y],[0,0,1]];
        that.mat = that.math.multiply(mat2, that.mat);
        var mat2Inv = [[1,0,-x],[0,1,-y],[0,0,1]];
    ...
    Trans2D.prototype.rotate = function(degrees) {
        ...
        var mat2 = [ [cosAngle,sinAngle,0], [-sinAngle,cosAngle,0], [0,0,1]];
        that.mat = that.math.multiply(mat2, that.mat);
        var mat2Inv = [ [cosAngle,-sinAngle,0], [sinAngle,cosAngle,0], [0,0,1]];
        that.matInv = that.math.multiply(that.matInv, mat2Inv);
    }
    Trans2D.prototype.shearX = function(degrees) {
        ...
        var mat2 = [ [1,tanAngle,0], [0,1,0], [0,0,1]];
        that.mat = that.math.multiply(mat2, that.mat);
        var mat2Inv = [ [1,-tanAngle,0], [0,1,0], [0,0,1]];
        that.matInv = that.math.multiply(that.matInv, mat2Inv);
    }
    Trans2D.prototype.shearY = function(degrees) {
        ...
        var radians = DEG_TO_RAD * degrees;
        var tanAngle = that.math.tan(-radians); // clockwise
        var mat2 = [ [1,0,0], [tanAngle,1,0], [0,0,1]];
        that.mat = that.math.multiply(mat2, that.mat);
...
(typeof describe === 'function') && describe("Trans2D", function() {
    ...
    it("translate(x,y) translates coordinate system by (x,y)", function() {
        ...
        assertxy(xfm.apply(1,2), {x:4,y:7}, e);
    })
    it("rotate(degrees) rotates coordinate system by clockwise degrees", function() {
        var xfm = new Trans2D();
        xfm.rotate(90);
        assertxy(xfm.apply(1,2), {x:2,y:-1}, e);
        xfm.rotate(90);
        assertxy(xfm.apply(1,2), {x:-1,y:-2}, e);
        xfm.rotate(90);
        assertxy(xfm.apply(1,2), {x:-2,y:1}, e);
        xfm.rotate(90);
        assertxy(xfm.apply(1,2), {x:1,y:2}, e);
    ...
    it("applyInverse(x,y) applies inverse transformation to point", function() {
        ...
        xfm.translate(3,5);
        assertxy(xfm.apply(1,2), {x:4,y:7}, e);
        assertxy(xfm.applyInverse(4,7), {x:1,y:2}, e);
        xfm.clear();
        xfm.rotate(90);
        assertxy(xfm.apply(1,2), {x:2,y:-1}, e);
        assertxy(xfm.applyInverse(2,-1), {x:1,y:2}, e);
        xfm.translate(3,5);
        assertxy(xfm.apply(1,2), {x:5,y:4}, e);
        assertxy(xfm.applyInverse(5,4), {x:1,y:2}, e);
    })
    it("shearX(degrees) shears coordinate system clockwise, parallel to x-axis", function() {
        var xfm = new Trans2D();
        xfm.shearX(45);
        assertxy(xfm.apply(1,0), {x:1,y:0}, e);
        assertxy(xfm.apply(1,1), {x:2,y:1}, e);
        assertxy(xfm.apply(1,2), {x:3,y:2}, e);
        assertxy(xfm.applyInverse(3,2), {x:1,y:2}, e);
    })
    it("shearY(degrees) shears coordinate system clockwise, parallel to y-axis", function() {
        var xfm = new Trans2D();
        xfm.shearY(-45);
        assertxy(xfm.apply(0,1), {x:0,y:1}, e);
