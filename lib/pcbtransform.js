...
(function(exports) {
    ...
    PcbTransform.prototype.writePng = function(transform) {
        var that = this;
        transform = transform || {};
        var pngPath = transform.png.path || "pcb.png";
        var svgWriter = new LogString();
        that.writeSvg(svgWriter, transform);
        var bufsvg = new Buffer(svgWriter.output);
        var width = transform.png.width || 800;
    ...
    PcbTransform.prototype.writeSvg = function(writer, options) {
        var that = this;
        options = options || {};
        ...
        if (that.gerber) {
            new PcbSvgFactory().create(writer, {
                ...
                arcs: pcb.pcbArcs(),
                holes: pcb.pcbHoles(),
                copperWires: pcb.pcbWires("GTL"),
                silkWires: pcb.pcbWires("GTO"),
...
(typeof describe === 'function') && describe("PcbTransform", function() {
    ...
    it("version() returns package.json version", function() {
        ...
        pcb.version().should.equal("0.1.1");
    })
    ...
    it("writeSvg(writer, options) writes SVG files for EagleBRD", function() {
        var slog = new LogString();
        var pcb = new PcbTransform();
        pcb.readEagleBrd("../eagle/test.brd");
        pcb.writeSvg(slog, {
    ...
    it("writePng(transform) writes PNG", function() {
        var pcb = new PcbTransform();
        pcb.readGerber({
            GKO: "../gerber/ruler/Ruler.GKO",
            GTP: "../gerber/ruler/Ruler.GTP",
        ...
        var buffer = new Buffer("zing");
        fs.writeFileSync("/tmp/test", buffer);
