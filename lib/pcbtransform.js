...
(function(exports) {
    ...
    PcbTransform.prototype.transform = function() {
        ...
        that.verbose && console.log("BEGIN\t: PCB file transformation in progress...");
        that.loadInputs();
        ...
        if (svg.path) {
            ...
            writer.close();
            that.verbose && console.log("SVG\t: generated SVG file", svg.path);
        }
        var csv = that.csv || {};
        if (csv.smdpads) {
            ...
            writer.close();
            that.verbose && console.log("CSV\t: generated CSV file", csv.smdpads);
        }
        if (csv.holes) {
            var writer = new LogFile(csv.holes);
            that.writeCsvHoles(writer);
            writer.close();
            that.verbose && console.log("CSV\t: generated CSV file", csv.holes);
        }
        var png = that.png || {};
        if (png.path) {
            png.width = png.width || 800;
            that.writePng();
            that.verbose && console.log("PNG\t: generated PNG file", png.path, "width:"+png.width);
        }
        that.verbose && console.log("END\t: Transform complete");
    }
    ...
    PcbTransform.prototype.readEagleBrd = function(path) {
        ...
        that.verbose && console.log("LOAD\t: Eagle file:"+path);
    }
    PcbTransform.prototype.readGerber = function(layerFiles) {
        ...
        for (var iLayer = 0; iLayer < layerKeys.length; iLayer++) {
            ...
            that.verbose && that.log("GERBER\t: Loaded "+path, "as", id, "file.",
