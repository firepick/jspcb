...
const StringLog = require("./stringlog");
...
(function(exports) {
    ...
    PcbTransform.prototype.writePng = function(pngPath, options) {
        var that = this;
        var svgWriter = new StringLog();
        options = JSON.parse(JSON.stringify(options || {}));
        options.writer = svgWriter;
        that.writeSvg(options);
        var buffer = svg2png.sync(new Buffer(svgWriter.output));
        that.verbose && console.log('writePng('+pngPath+')', buffer.length,'bytes');
...
(typeof describe === 'function') && describe("PcbTransform", function() {
    ...
    it("version() returns package.json version", function() {
        ...
        pcb.version().should.equal("0.1.0");
    })
    ...
    it("writePng(pngPath, options) writes PNG", function() {
        var pcb = new PcbTransform();
        pcb.readGerber({
            GKO: "../gerber/ruler/Ruler.GKO",
            GTP: "../gerber/ruler/Ruler.GTP",
        ...
        try { fs.statSync(pngTmp); fs.unlinkSync(pngTmp); } catch (e) {}
        pcb.writePng("/tmp/pcb.png");
        var stats = fs.statSync(pngTmp);
        stats.size.should.equal(2331);
    });
})
