...
(function(exports) {
    ...
    PcbTransform.prototype.pcbBounds = function() {
        ...
        if (bounds.t == null || bounds.l == null || bounds.r == null || bounds.b == null) {
            var svgFactory = new PcbSvgFactory();
            var brd = that.getPcb();
            bounds = that.bounds = svgFactory.pcbBounds({
                dimWires: that.pcbWires("Dimension"),
            });
        }
        return bounds;
    }
    ...
    PcbTransform.prototype.transform = function() {
        ...
        if (csv.smdpads) {
            ...
            writer.close();
            that.verbose && console.log("WRITE\t: "+csv.smdpads);
        }
        if (csv.holes) {
            var writer = new LogFile(csv.holes);
            that.writeCsvHoles(writer);
            writer.close();
            that.verbose && console.log("WRITE\t: "+csv.holes);
        }
        var png = that.png || {};
        if (png.path) {
            that.writePng();
    ...
    PcbTransform.prototype.readGerber = function(layerFiles) {
        ...
        for (var iLayer = 0; iLayer < layerKeys.length; iLayer++) {
            ...
            that.verbose && that.log("LOAD\t: Gerber", id, "file:"+path,
    ...
    PcbTransform.prototype.writePng = function() {
        ...
        that.verbose && console.log("WRITE\t: "+pngPath, 
...
(typeof describe === 'function') && describe("PcbTransform", function() {
    ...
    it("pcbBounds() returns PCB bounding box (Gerber)", function() {
        ...
        pcbTrans.transform();
        slog.output.should.equal('{"r":180.34,"l":25.4,"b":25.4,"t":50.8}\n');
    });
