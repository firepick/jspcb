...
(function(exports) {
        ...
        switch (dcode) {
            case 1: // D01 interpolate
                if (layer.interpolation === "L") {
                    ...
                    if (dx === 0 && dy === 0) {
                        throw new Error("[GRB014] I and J cannot both be zero:" + datablock.s);
                    }
                    layer.graphics.push(that.svgArc( layer.x, layer.y,
                        x2, y2, dx, dy, layer.interpolation==="CW", layer));
                }
                break;
            case 2: // D02 move
                break;
    ...
    Gerber.prototype.svgArc = function(x1, y1, x2, y2, dx1, dy1, cw, layer) {
        var that = this;
        var cx = x1 + dx1;
        var cy = y1 + dy1;
        ...
        var angle = 0;
        that.verbose && console.log("Arc center("+cx+","+cy+")");
        var aperture = layer && layer.aperture || {};
        if (aperture.type === "circle") {
            var width = aperture.r * 2;
        } else if (aperture.type === "rect") {
            var width = Math.max(aperture.width, aperture.height);
        } else if (aperture.type === "polygon") {
            var width = aperture.od;
        } else {
            var width = 1;
        }
    ...
    Gerber.prototype.pcbHoles = function(layerId) {
        var that = this;
        var priority = [
            layerId, // user chosen layer
            "TXT", // normal drill layer
            "GTS", // soldermask circles (Altium/Protel?)
        ];
...
(typeof describe === 'function') && describe("Gerber", function() {
    ...
    it("svgArc(x1,y1,x2,y2,dx,dy,ccw,layer) returns SVG arc parameters for given Gerber arc", function() {
        var grb = new Gerber({
            verbose: false
        });
        var e = .0001;
        ...
        // horizontal
        var arc = grb.svgArc(1, 0, 3, 0, 1, -1, true, layer);
    ...
    it("pcbArcs(layerSpec) returns arcs in specified layer", function() {
        ...
        gtoArcs[0].width.should.approximately(25.4 * 0.137, e);
