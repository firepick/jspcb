...
(function(exports) {
    ...
    function Gerber(options) {
        ...
        options = options || {};
        that.strokeWidth = options.strokeWidth == null ? 0.1 : options.strokeWidth;
        that.layers = {};
    ...
    function processExtendedCode(that, layer, datablock) {
                ...
                if (datablock.peekCharCode(CHARP)) {
                    ...
                    if (datablock.peekCharCode(CHARE)) {
                        if (datablock.peekCharCode(CHARG)) {
                            that.polarity = true;
                            console.log("[GRB004] ERROR: unsupported and deprecated extended function code:", datablock.s);
                            parsed = true;
                        }
                    }
                }
        ...
        if (!parsed) {
            console.log("[GRB005] ignoring non-standard extended function code:" + JSON.stringify(datablock));
        }
    }
            ...
            case 1: // D01 interpolate
                if (layer.interpolation === "L") {
                    layer.graphics.push({
                        ...
                        x2: x2,
                        y2: y2,
                        sw: layer.aperture && layer.aperture.sw || that.strokeWidth,
                    });
                } else {
                    if (dx === 0 && dy === 0) {
    ...
    Gerber.prototype.pcbWires = function(layerId) {
        var that = this;
        layerId = that.layerId(layerId, ["GKO", "GTL", "GTO", "GBL", "GBO"]);
