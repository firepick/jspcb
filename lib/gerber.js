...
(function(exports) {
                ...
                if (datablock.peekCharCode(CHARD)) {
                    ...
                    if (name === CHARC) {
                        if (datablock.peekCharCode(CHARCOMMA)) {
                            var aperture = {
                                type: "C",
                                diameter: layer.unit * datablock.getDouble(),
                            }
                        ...
                        if (datablock.peekCharCode(CHARCOMMA)) {
                            ...
                            if (datablock.peekCharCode(CHARX)) {
                                var aperture = {
                                    type: "O",
                                    w: layer.unit * x,
                                    h: layer.unit * datablock.getDouble(),
                                }
                                if (datablock.peekCharCode(CHARX)) {
                                    aperture.hole = datablock.getDouble();
                                }
    ...
    function processDCode(that, layer, datablock) {
        ...
        case 1: // D01 interpolate
            layer.x = x + dx;
            layer.y = y + dy;
            console.log("Ignoring D01 interpolation", datablock);
            break;
        case 2: // D02 move
            layer.x = x + dx;
            layer.y = y + dy;
        ...
        case 3: // D03 flash
            ...
            var aperture = JSON.parse(JSON.stringify(layer.aperture));
            aperture.x = layer.x;
            aperture.y = layer.y;
            layer.graphics.push(aperture);
            break;
         default:
...
(typeof describe === 'function') && describe("Gerber", function() {
    ...
    it("parseLayer(id,stringOrBuffer) parses A commands", function() {
        ...
        var layer = grb.parseLayer("GTO", "%ADD0078P,10X5X-2.3X3.4*%");
        should.deepEqual(layer.apertures[78], {
            type: "P",
            od: 10,
            v: 5,
        ...
        var layer = grb.parseLayer("GTO", "%ADD0078P,10X5X-2.3*%");
        should.deepEqual(layer.apertures[78], {
            type: "P",
            od: 10,
            v: 5,
    ...
    it("parseLayer(id,stringOrBuffer) parses D commands", function() {
        ...
        var layer = grb.parseLayer("GTO", "%FSLAX12Y12*% %MOMM*% I2J3X123Y456D2*");
        layer.x.should.approximately(1.25, e);
        layer.y.should.approximately(4.59, e);
