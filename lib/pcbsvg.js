(function(exports) {
    ...
    PcbSvgFactory.prototype.pcbArcs = function(arcs) {
        var that = this;
        if (!arcs || !arcs.length) {
            return that;
        }
    ...
    PcbSvgFactory.prototype.create = function(options) {
        ...
        for (var iWire = 0; iWire < dimWires.length; iWire++) {
            ...
            that.element("line", {
                ...
                y2: -wire.y2,
                width: 0.5,
                stroke: that.colors.outline || "#f0f",
            }, "dimension");
        }

        that.pcbTexts(options.texts || []);
        that.pcbArcs(options.arcs || []);
...
(typeof describe === 'function') && describe("EagleBRD", function() {
    ...
    it("pcbArcs(arcs) creates SVG document with silkscreen arcs", function() {
        var slog = new LogString();
        var factory = new PcbSvgFactory({writer: slog});
        var arcs = factory.pcbArcs([]);
        ...
        slog.output.should.equal(
            ...
            '<path d="M2 3, A 6 7, 0 1 0 4 5 "/>\n' +
            '<path d="M20 30, A 60 70, 0 0 1 40 50 "/>\n' +
            '</g><!--pcb arcs-->\n' );
    })
});
