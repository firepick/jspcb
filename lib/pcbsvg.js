(function(exports) {
    ...
    PcbSvgFactory.prototype.calcViewBox = function(options) {
        var that = this;
        options = options || {};
    ...
    PcbSvgFactory.prototype.create = function(writer, options) {
        var that = this;
        options = options || {};
        ...
        var bounds = that.calcViewBox(options);
        var width = Math.abs(bounds.r - bounds.l);
        var height = Math.abs(bounds.b - bounds.t);
        that.log('<svg xmlns="http://www.w3.org/2000/svg" version="1.1"',
            'width="' + width + 'mm" height="' + height + 'mm"',
            'viewbox="',
            bounds.l, -Math.max(bounds.b, bounds.t), width, height,
            '" >');
        ...
        that.writeWires(dimWires, "dimension", colors.outline || "#f0f");
...
(typeof describe === 'function') && describe("EagleBRD", function() {
    ...
    it("calcViewBox(options) returns SVG view box", function() {
        var slog = new LogString();
        var factory = new PcbSvgFactory({
            writer: slog
        });
        var bounds = factory.calcViewBox({
            dimWires: dimWires
        });
    ...
    it("create(writer, options) creates SVG document with PCB outline", function() {
        var slog = new LogString();
        var factory = new PcbSvgFactory();
        factory.create(slog, {
            dimWires: dimWires
        ...
        slog.output.should.equal(SVGHEADER +
            ...
            '<g stroke="#f0f" fill="none" stroke-width="0.1" transform="scale(1,-1)"><!--dimension-->\n' +
